...
class Controller extends BaseController
{
    ...
    {
        $p_hot = Product::query()->where('status', Status::ACTIVE)
            ->where('price', '<', 25000000)->orderBy('price', 'DESC')->take(6)->get();
        ...
        $p_midrange = Product::query()->where('status', Status::ACTIVE)
            ->where('price', '<', 12000000)->where('price', '>', 5000000)
