    ...
    <div class="col-md-9">
        ...
        @section('input_form')
                ...
                <div class="col-lg-6">
                    ...
                    <select name="category_id" id="" class="form-control">
                        @foreach($categories as $item)
                            <option
                                {{$detail && $detail->category_id == $item->id ?'selected' :''}} value="{{$item->id}}">{{$item->name}}</option>
                        @endforeach
                    </select>
                </div>
            </div>
                ...
                <div class="col-lg-6">
                    ...
                    <select name="brand_id" id="" class="form-control">
                        @foreach($brands as $item)
                            <option
                                {{$detail && $detail->brand_id == $item->id ?'selected' :''}} value="{{$item->id}}">{{$item->name}}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            <div class="row form-group">
                ...
                <div class="col-lg-6">
                    <label>Price</label>
                    <input value="{{$detail ? $detail->price : ''}}" name="price" type="text"
                           placeholder="Vui lòng nhập giá co sản phẩm này" class="form-control">
                </div>
                <br>
                <div class="col-lg-12 row show_images_product" style="margin-top: 30px">
                    @if($detail)
                        @for($i=0;$i<sizeof(json_decode($detail->images,true));$i++)
                            <div id="image{{$i}}" class="col-md-3"
                                 style="height: 200px;overflow: hidden;margin-bottom: 10px">
                                <img class="product_image"
                                     style="width: 100%;height: 160px;object-fit: cover;border-radius: 5px;margin-bottom: 2px"
                                     src="{{json_decode($detail->images,true)[$i]}}" alt="">
                                <button type="button" slot="image{{$i}}" onclick="delete_image(this.slot)"
                                        class="btn btn-info form-control">Close
                                </button>
                            </div>
                        @endfor
                    @endif
            ...
            <div class="row form-group">
                <div class="col-lg-12">
                    ...
                    <textarea style="height: 100px" name="description" type="text" placeholder="Enter description"
                              class="form-control">{{$detail ?$detail->description :''}}</textarea>
                </div>
            </div>
            <div class="row form-group">
                <div class="col-lg-12">
                            ...
                            @if($key == 0)
                                <div class="option">
                                    ...
                                    <div class="row form-group">
                                        <div class="col-lg-7">
                                            ...
                                            <button type="button" onclick="upload_image_option('0987987')"
                                                    class="btn btn-danger form-control Choose_thumbnail_option">Choose
                                                thumbnail
                                            </button>
                                        </div>
                                        <div class="col-lg-5">
                                            <img src="{{$item->thumbnail}}" alt="" class="demo_option_thumbnail"
                                                 id="0987987img">
                                        </div>
                                    </div>
                                    <div class="row form-group">
                                        ...
                                        <div class="col-lg-4">
                                            ...
                                            <select name="" id="" class="form-control color">
                                                @foreach($colors as $items)
                                                    <option
                                                        {{$item->color_id == $items->id ? 'selected' : ''}} value="{{$items->id}}">{{$items->name}}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                    <div class="row form-group">
                                        <div class="col-lg-4">
                                            <label>Ram (GB)</label>
                                            <input type="number" class="form-control ram" value="{{$item->ram}}"
                                                   min="2">
                                        </div>
                                        <div class="col-lg-4">
                                            <label>Rom (GB)</label>
                                            <input type="number" class="form-control rom" value="{{$item->rom}}"
                                                   min="16">
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="option" id="close{{$key}}">
                                    ...
                                    <div class="row form-group">
                                        <div class="col-lg-7">
                                            <label for="Discount">Choose thumbnail</label>
                                            <button type="button"
                                                    onclick="upload_image_option('upload_image_id_{{$key}}')"
                                                    class="btn btn-danger form-control Choose_thumbnail_option">Choose
                                                thumbnail
                                            </button>
                                        </div>
                                        <div class="col-lg-5">
                                            <img src="{{$item->thumbnail}}" alt="" class="demo_option_thumbnail"
                                                 id="upload_image_id_{{$key}}img">
                                            <span onclick="close_option('close{{$key}}')" class="btn_close"
                                                  title="close this option">&times;</span>
                                        </div>
                                    </div>
...
`)
                    ...
                    $('.btn_check_and_create_data').click(function () {
                        ...
                        var slug = document.querySelector('.product_name').$('#thumbnail').val(product_images[0].src)
